...
public class Feeling implements Serializable {
	...
	public static final class Feelings implements BaseColumns {
		...
		public static List<Feeling> getAll(Context c) {
			Cursor cursor = c.getContentResolver().query(Feelings.CONTENT_URI,
					new String[] {Feelings.VALUE, Feelings.CREATED, Feelings.NOTE }, null, null, null);
			List<Feeling> feelings = new ArrayList<Feeling>();
			while (cursor.moveToNext()) {
				Feeling feeling = new Feeling(cursor.getInt(0), cursor.getLong(1), cursor.getString(2));
